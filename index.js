...
client.connect().then(() => {
  ...
  foodCollection = client.db("GubConnect").collection("Food");
  HelpPostCollection = db.collection("HelpPost");
});
...
async function run() {
  try {
    ...
    const JobPostCollection = db.collection("Job");
    ...
    app.post("/api/help", async (req, res) => {
      ...
      try {
        await HelpPostCollection.insertOne({
          userId,
          text,
          timestamp: new Date(),
        });
        res.status(201).json({ message: "Help posted successfully" });
      } catch (error) {
        console.error("Error posting help:", error.message);
        res.status(500).json({ error: "Internal server error" });
      }
    });
    app.post("/api/help/:postId/reply", async (req, res) => {
      const { userId, text } = req.body;
      const postId = req.params.postId;

      try {
        await HelpPostCollection.updateOne(
          { _id: new ObjectId(postId) }, // Instantiate ObjectId with 'new' keyword
          {
            $push: {
              replies: {
                userId,
    ...
    // API endpoint to fetch help posts
    app.get("/api/help", async (req, res) => {
      try {
        const helpPosts = await HelpPostCollection.find()
          .sort({ timestamp: -1 })
          .toArray();
        res.json(helpPosts);
      } catch (error) {
        console.error("Error fetching help posts:", error.message);
        res.status(500).json({ error: "Internal server error" });
      }
    });
    ...
    app.post("/api/foodItems", async (req, res) => {
      try {
        ...
        const addedFoodItem = await foodCollection.findOne({
          _id: result.insertedId,
        });

        res.status(201).json(addedFoodItem);
      } catch (error) {
    ...
    app.delete("/api/resetFood", async (req, res) => {
      try {
        await foodCollection.deleteMany({});
        res.status(204).send();
      } catch (error) {
        console.error("Error resetting food items:", error);
        res.status(500).json({ error: "Internal Server Error" });
      }
    });
    app.post(
    ...
    app.post("/send-email", async (req, res) => {
      ...
      // Check if 'to' field is empty or not defined
      if (!to) {
        console.warn(
          "Warning: Recipient email address is not provided. Skipping email."
        );
        return res
          .status(200)
          .send("Recipient email address not provided. Email skipped.");
      }
          ...
          if (user.Role === "Student") {
            ...
            if (Password === user.Password) {
              ...
              if (user.Status === "Approved") {
                // Generate JWT token
                const token = jwt.sign({ S_id }, secretKey, {
                  expiresIn: "1h",
                });
                console.log(token);
            ...
            res.json({ success: false, message: "User is not a Student" });
          }
        } else {
          res.json({ success: false, message: "Invalid credentials" });
        }
      ...
      try {
        ...
        if (user) {
          ...
          if (user.Role === "Alumni") {
            ...
            res.json({ success: false, message: "User is not a Alumni" });
          }
        } else {
          res.json({ success: false, message: "Invalid credentials" });
        }
    ...
    app.put("/updatefaculty/:id", async (req, res) => {
      try {
        ...
        // Validation
        if (!name || !email || !department || !freetime) {
          return res.status(400).json({
            error: "Name, email, password, and freetime are required fields.",
          });
        }
      ...
      async (req, res) => {
        try {
          const {
            ...
            phone,
            expertise,
            freetime, // This is the new field for Counseling Hours
          } = req.body;

          // Validation
          if (!name || !email || !password || !freetime) {
            return res.status(400).json({
              error: "Name, email, password, and freetime are required fields.",
            });
          }
          ...
          const newFaculty = {
            ...
            phone,
            expertise,
            freetime, // Adding freetime to the newFaculty object
    ...
    app.post("/addJob", upload.array("Images"), async (req, res) => {
      try {
